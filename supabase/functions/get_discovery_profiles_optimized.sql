-- Optimized RPC function that primarily uses filter_preferences table for filtering
-- This function always tries to use saved preferences first, with fallback to parameters
...
BEGIN
  ...
  IF current_user_gender IS NULL THEN
    RAISE NOTICE 'User profile not found for ID: %', user_id;
    RETURN;
  END IF;
  ...
  -- If no saved preferences found, use sensible defaults
  IF NOT FOUND THEN
    RAISE NOTICE 'No filter preferences found for user %, using defaults', user_id;
  ...
  ELSE
    -- Use defaults for NULL values in saved preferences
    filter_interested_in := COALESCE(filter_interested_in, 'any');
    filter_min_age := COALESCE(filter_min_age, 18);
    filter_max_age := COALESCE(filter_max_age, 35);
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error in get_discovery_profiles_optimized: %', SQLERRM;
    RETURN;
END;
$$;
...
-- Create a wrapper that maintains the original interface but uses filter_preferences
CREATE OR REPLACE FUNCTION get_discovery_profiles_from_preferences(
  user_id UUID
)
RETURNS TABLE (
  id UUID,
...
AS $$
BEGIN
  -- Simply call the optimized function
  RETURN QUERY
  SELECT * FROM get_discovery_profiles_optimized(user_id);
END;
$$;

GRANT EXECUTE ON FUNCTION get_discovery_profiles_from_preferences TO authenticated;
